var passenger_form = {
  ...
  'init': function(){
    ...
    $('#amount').on('input', function(){
      ...
      val = val<passenger_form.maxAmount?val:passenger_form.maxAmount;
      $(this).val(val);
      ...
      if (val > crntMaxAmount) {
        ...
        if (val > passenger_form.stdMaxAmount) {
          $('#slider-for-price').slider('option', 'max', val);
          $('.firstRangeSlider.amount label.max').html(val);
        } else {
          $('#slider-for-price').slider('option', 'max', passenger_form.stdMaxAmount);
          $('.firstRangeSlider.amount label.max').html(passenger_form.stdMaxAmount);
        }
      }
    ...
    $('#slider-for-price').slider({
      ...
      value: 1,
      min: 1,
      max: passenger_form.stdMaxAmount,
    ...
    $('form.itw-multistep-form').on('submit.passenger_form', function(e){
      e.preventDefault();
      var data = passenger_form.get_data();
      if (passenger_form.validate(data)) {
        data.action = 'create';
  ...
  'get_data': function(){
    ...
    var data = {
      'short-path' : passenger_form.get_short_path(),
      'path'       : passenger_form.get_path(),
      'start-location' : {
  ...
  'validate': function(data){
    var result = passenger_form_validator.run(data);
    if (!result) {
...
var passenger_form_validator = {
  run: function(data){
    ...
    for(var i in data) {
      ...
      if (passenger_form_validator[method_name] instanceof Function) {
        // console.log('Validating: '+i+': ');
        // console.log(data[i]);
        ok = passenger_form_validator[method_name](data[i]) && ok;
      } else {
  ...
  'validate_email': function(data){
    ...
    passenger_form_validator.setError('email');
    return false;
  },
  'validate_path': function(data){
    if (data.length > 0) return true;
    passenger_form_validator.setError('path');
    return false;
  },
  'validate_start-location': function(data){
    if (data.id) return true;
    passenger_form_validator.setError('start-location');
    return false;
  },
  'validate_end-location': function(data){
    if (data.id) return true;
    passenger_form_validator.setError('end-location');
    return false;
  },
  'validate_startAt': function(data){
    ...
    passenger_form_validator.setError('startAt');
    return false;
  },
  'validate_returnAt': function(data){
    ...
    passenger_form_validator.setError('returnAt');
    return false;
  },
  'validate_name': function(data){
    if (data.length > 2) return true;
    passenger_form_validator.setError('name');
    return false;
  },
  'validate_phone': function(data){
    var re = /^[+#*\(\)\[\]]*([0-9][ ext+-pw#*\(\)\[\]]*){6,45}$/i;
    if (re.test(data)) return true;
    passenger_form_validator.setError('phone');
    return false;
  },
  'setError': function(el){
    // console.log('error: '+el);
    $('.validate-el-'+el).addClass('error');
    $('.validate-error-'+el).html(passenger_form_lang['validate_'+el] || '').show();
  }
}
