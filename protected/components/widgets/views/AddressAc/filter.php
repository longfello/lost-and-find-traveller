...
<script type="text/javascript">
  $(window).ready(function(){
    ...
    <?php if ($this->parent) { ?>
      var chlds = $('.<?=$this->parent?>').data('child') || [];
      chlds.push($('#<?=$el_id?>'));
       $('.<?=$this->parent?>').data('child', chlds);

      $(input)
        .on('setBounds', function(a,b){
          autocomplete_<?=$el_no?>.setBounds(b.bounds);
        })
        .on('blur', function(){
            setTimeout(function(){
            $(input).val($(input).data('name')).inputfit({minSize : 12, maxSize : 24});
          }, 500);
        })
    ...
    google.maps.event.addListener(autocomplete_<?=$el_no?>, 'place_changed', function() {
      ...
      setTimeout(function(){
        $(input).val(place.name || place.short_name).inputfit({minSize : 12, maxSize : 24});
      }, 500);

      $(input).data({
        name: place.name || place.short_name,
