...
class CmsController extends Controller
{
	...
	public function accessRules()
	{
		return array(
			array('allow',  // allow all users to perform 'index' and 'view' actions
				'actions'=>array('view','group'),
				'users'=>array('*'),
			),
			array('allow', 
				'actions'=>array('index', 'create','update', 'admin','delete','sort','translit','getParentUrl'),
				'users'=>array('admin'),
			),
	...
	public function actionGroup()
	{
		...
		$category = ArticleCategory::model()->find("url = :url", array("url"=>$catName));
		if(empty($category)){
			throw new CHttpException(404, "Category $catName not found");
		} 

	
		$allArs = Cms::model()->findAll("category = :category", array("category"=>$category->id_cat) );
		$this->render('by_category',array(
			'orders'=>$allArs,'category'=>$category,
		));
    ...
    if ($_GET['parent_id']) {
      ...
      if( $model->validate() && $parentNode->appendChild($model)) {
		...
		$this->render('create',array(
			'model'=>$model,'ArticleCategory'=>$ArticleCategory,
		));
	}
	...
	{
	//$model=$this->loadModel($id);
		$model=Cms::model()->with( array('idCat') )->findByPk($id);	
