<tr class="<?=($index % 2)?'odd':'even' ?>">
	...
	<td>
                <?
                        switch($data->bnrTyp)
                        {
                                case 'img' : { $r='<img width="'.$data->bnrWidth.'" height="'.$data->bnrHeight.'" src="'.Yii::app()->params['bannersWebFolder'].$data->bnrFile.'" title="'.htmlentities($data->bnrDescr).'" alt="'.htmlentities($data->bnrDescr).'">'; break; }
	...
	<td>
                ...
                &nbsp;|&nbsp;
                <?=CHtml::link('Copy',array('banners/copyBanner','id'=>$data->id));?>
        </td>
</tr>